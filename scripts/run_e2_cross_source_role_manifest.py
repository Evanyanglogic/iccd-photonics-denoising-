...
def main() -> int:
    ...
    run = {"experiment_id": config["experiment_id"], "started_at_utc": started, "ended_at_utc": now(), "git_commit": commit, "status": final_status, "previous_failed_run": {"path": "reports/e2_cross_source_role_manifest_20260718", "status": "FAILED", "reason": "OVERBROAD-HISTORICAL-TRAINING-FLAG", "reused_as_formal_result": False}, "source_data_protected": source_protected, "source_write_performed": False, "synthetic_pairs_generated": False, "training_or_split_performed": False, "pmrid_file_count_before": pmrid_count_before, "pmrid_file_count_after": len(pmrid_files_after), "scmos_file_count_before": scmos_count_before, "scmos_file_count_after": len(list(scmos_dir.glob("*.tif*")))}
