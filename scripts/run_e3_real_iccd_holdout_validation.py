...
def main() -> int:
    ...
    for folder in selected_folders:
        ...
        for index, images in preview_cache.items():
            ...
            for name in ["raw", "G", "CG_NC", "CG_C"]: save_preview(out / f"previews/folder_{folder}/frame_{index + 1:03d}_{name}.png", images[name], low, high)
            for name in ["G", "CG_NC", "CG_C"]: save_preview(out / f"previews/folder_{folder}/frame_{index + 1:03d}_removed_{name}.png", images[f"removed_{name}"], -residual_limit, residual_limit)
        del raw_stack
    ...
    comparisons = []
    for folder in selected_folders:
        for model_name in ["G", "CG_NC", "CG_C"]:
