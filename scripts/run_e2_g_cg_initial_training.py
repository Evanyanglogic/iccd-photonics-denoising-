...
def main():
 ...
 freeze=subprocess.run([sys.executable,'-m','pip','freeze'],text=True,capture_output=True,check=True).stdout; (prov/'pip_freeze.txt').write_text(freeze,encoding='utf-8')
 gpu=subprocess.run(['nvidia-smi'],text=True,capture_output=True,check=False); (prov/'gpu_info.txt').write_text(gpu.stdout if gpu.returncode==0 else f'unavailable: {gpu.stderr}',encoding='utf-8')
