...
def run(repo: Path, cfg: dict[str, Any], output: Path, source_config: Path) -> int:
    ...
    for index, row in enumerate(historical):
        ...
        thumbnails.append(build_thumbnail_record(row["source_pair_key"], roi, int(settings["thumbnail_size"])))
    ...
    status = verify(cfg, commit, git_status, hash_rows, metadata_rows, full_rows, roi_rows, duplicate_rows, group_summary, processing)
    ...
    run_manifest = {
        ...
        "started_and_completed_at_utc": utc_now(),
...
def statistics_row(pair_key: str, path: Path, image: np.ndarray, scope: str, coords: tuple[int, int, int, int]) -> dict[str, Any]:
    ...
    q = np.percentile(values, [0.1, 1, 5, 50, 95, 99])
    ...
    return {
        ...
        "raw_p1_dn": float(q[1]),
        "raw_p5_dn": float(q[2]),
        "raw_p50_dn": float(q[3]),
        "raw_p95_dn": float(q[4]),
        "raw_p99_dn": float(q[5]),
        ...
        "norm_p50": float(q[3] / 65535.0),
        "norm_p95": float(q[4] / 65535.0),
        "norm_p99": float(q[5] / 65535.0),
        ...
        "robust_dynamic_range_dn_p1_p99": float(q[5] - q[1]),
...
def build_thumbnail_record(pair_key: str, roi: np.ndarray, size: int) -> dict[str, Any]:
    ...
    high = thumb - low
    return {"pair_key": pair_key, "image": thumb, "low": low, "high": high}
...
def verify(
    ...
    add("file_count_100", len(manifest) == 100, len(manifest))
    ...
    add("metadata_recorded", len(metadata) == 100 and all(row["all_tags_json"] for row in metadata), len(metadata))
    add("clean_commit", not git_status.strip(), commit)
...
def write_provenance(repo: Path, cfg: dict[str, Any], source_config: Path, output: Path, commit: str, status: str) -> None:
    ...
    write_text(output / "git_status.txt", status)
...
def duplicate_fields() -> list[str]:
    return ["source_pair_key_a", "source_pair_key_b", "center_roi_correlation", "center_roi_ssim", "low_frequency_correlation", "high_frequency_correlation", "high_content_similarity", "duplicate_type"]
