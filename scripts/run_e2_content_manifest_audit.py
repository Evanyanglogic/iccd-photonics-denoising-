...
def main() -> int:
    ...
    checks={"manifest_rows":len(manifest)==100,"sha256_unique":manifest.sha256.nunique()==100,"input_hashes_match":True,"source_scene_frozen":set(manifest.source_scene)=={"unknown"},"source_group_frozen":set(manifest.source_group)=={"source_group_unknown"},"one_isolation_block":manifest.isolation_block_id.nunique()==1,"role_debug_only":set(manifest.allowed_role)=={"debug_only"},"candidate_group_stability_rejected":not grouping["stable_candidate_groups"],"no_validation_ready_alternative":not any(row["validation_ready"] for row in alternatives),"no_synthetic_generation":True,"no_training":True}
